...
#define __KAVL_ERASE(suf, __scope, __type, __head, __cmp) \
    __scope __type *kavl_erase_##suf(__type **root_, const __type *x, unsigned *cnt_) { __type *p, *path[KAVL_MAX_DEPTH], fake; unsigned char dir[KAVL_MAX_DEPTH]; int i, d = 0, cmp; unsigned cnt = 0; fake.__head.p[0] = *root_, fake.__head.p[1] = 0; if (cnt_) *cnt_ = 0; if (x) { for (cmp = -1, p = &fake; cmp; cmp = __cmp(x, p)) { int which = (cmp > 0); if (cmp > 0) cnt += kavl_size_child(__head, p, 0) + 1; if (d >= KAVL_MAX_DEPTH) return 0; dir[d] = which; path[d++] = p; p = p->__head.p[which]; if (p == 0) { if (cnt_) *cnt_ = 0; return 0; } } cnt += kavl_size_child(__head, p, 0) + 1; } else { for (p = &fake, cnt = 1; p; p = p->__head.p[0]) { if (d >= KAVL_MAX_DEPTH) break; dir[d] = 0, path[d++] = p; } if (d > 0) p = path[--d]; else return 0; } if (cnt_) *cnt_ = cnt; for (i = 1; i < d; ++i) --path[i]->__head.size; if (p->__head.p[1] == 0) { if (d > 0) path[d-1]->__head.p[dir[d-1]] = p->__head.p[0]; } else { __type *q = p->__head.p[1]; if (q->__head.p[0] == 0) { q->__head.p[0] = p->__head.p[0]; q->__head.balance = p->__head.balance; if (d > 0) path[d-1]->__head.p[dir[d-1]] = q; path[d] = q, dir[d++] = 1; q->__head.size = p->__head.size - 1; } else { __type *r; int e = d++; for (;;) { if (d >= KAVL_MAX_DEPTH) break; dir[d] = 0; path[d++] = q; r = q->__head.p[0]; if (r->__head.p[0] == 0) break; q = r; } r->__head.p[0] = p->__head.p[0]; q->__head.p[0] = r->__head.p[1]; r->__head.p[1] = p->__head.p[1]; r->__head.balance = p->__head.balance; if (e > 0) path[e-1]->__head.p[dir[e-1]] = r; path[e] = r, dir[e] = 1; for (i = e + 1; i < d; ++i) --path[i]->__head.size; r->__head.size = p->__head.size - 1; } } while (--d > 0) { __type *q = path[d]; int which, other, b1 = 1, b2 = 2; which = dir[d], other = 1 - which; if (which) b1 = -b1, b2 = -b2; q->__head.balance += b1; if (q->__head.balance == b1) break; else if (q->__head.balance == b2) { __type *r = q->__head.p[other]; if (r->__head.balance == -b1) { if (d > 0) path[d-1]->__head.p[dir[d-1]] = kavl_rotate2_##suf(q, which); } else { if (d > 0) path[d-1]->__head.p[dir[d-1]] = kavl_rotate1_##suf(q, which); if (r->__head.balance == 0) { r->__head.balance = -b1; q->__head.balance = b1; break; } else r->__head.balance = q->__head.balance = 0; } } } *root_ = fake.__head.p[0]; return p; }
...
#define __KAVL_ITR(suf, __scope, __type, __head, __cmp) \
    ...
    __scope void kavl_itr_first_##suf(const __type *root, struct kavl_itr_##suf *itr) { const __type *p; itr->top = itr->stack - 1; for (p = root; p; p = p->__head.p[0]) *++itr->top = p; if (itr->top >= itr->stack) itr->right = (*itr->top)->__head.p[1]; else itr->right = 0; } \
